...
export default function NextEventsMap({ data }) {
  ...
  return (
      ...
      style={{ backgroundImage: `linear-gradient(rgba(255, 130, 46, 0.6), rgba(255, 130, 46, 0.6)), url(${data.image_url})` } }
        className={`border-4 border-citrus bg-cover bg-center rounded-2xl mt-5 mb-3 p-5 w-[470px]`} // give a width and height
      >
        ...
        <p className="text-black font-bold text-base">{`${data.event_time.substring(0, 5)} - ${data.end_time.substring(
          0,
          5
        )}`}</p>
        </a>
